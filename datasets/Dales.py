...
from utils.ply import read_ply_file_all_formats
...
class DalesDataset(PointCloudDataset):
    ...
    def __init__(self, config, set='training', use_potentials=True, load_data=True):
        ...
        # Dataset folder
        self.path = '/media/mmm/8cb82fdb-5c41-4d04-8883-a5d6e2fece9b/hirsch/dales_ply'
        ...
        self.cloud_names = list(map(basename, glob(join(ply_path, '*'))))
    ...
    def prepare_Dales_ply(self):
        ...
        for cloud_name in self.cloud_names:
            ...
            cloud_file = join(ply_path, cloud_name)
    ...
    def load_subsampled_clouds(self):
        ...
        for i, file_path in enumerate(self.files):
            ...
            sub_ply_file = join(tree_path, '{:s}'.format(cloud_name))
            ...
            if exists(KDTree_file):
                ...
                # read ply with data
                data = read_ply_file_all_formats(sub_ply_file)
                sub_colors = (data.drop(['x', 'y', 'z', 'class'], axis=1)).to_numpy()
                sub_labels = data['class'].to_numpy()
                ...
                # Read ply file
                data = read_ply_file_all_formats(file_path)
                points = np.array(np.vstack((data['x'], data['y'], data['z'])), dtype=float).T
                # colors = np.vstack((data['red'], data['green'], data['blue'])).T
                colors = np.ones((len(points), 1), dtype=np.float32)
                labels = data['class'].to_numpy()

                # pc_down_sampled, _, pc_indexes_down_sampled = voxel_downsamle(points, 1)
                # sub_colors = apply_sampling(colors, pc_indexes_down_sampled, np.average)
                # sub_labels = apply_sampling(labels, pc_indexes_down_sampled, most_frequent)

                # Subsample cloud
                sub_points, sub_colors, sub_labels = grid_subsampling(points.astype(np.float32),
                ...
                search_tree = KDTree(np.array(sub_points), leaf_size=10)
    ...
    def load_evaluation_points(self, file_path):
        ...
        data = read_ply_file_all_formats(file_path)
        data = np.array(data)
        return np.vstack((data[:,0], data[:,1], data[:,2])).T
